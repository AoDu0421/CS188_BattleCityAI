...
class Level():
	...
	def loadLevel(self, level_nr = 35):
		""" Load specified level
		@return boolean Whether level was loaded
		"""
		filename = "levels/"+str(22)
...
class Enemy(Tank):
		...
		while not fringe.isEmpty():
			...
			for curNode,curAction in self.getSuccessors(node):
				if not curNode in visited:
					x,y = curNode
					if (x < target_position[0] + check_unit) and (x > target_position[0] -check_unit) and (y < target_position[1]+check_unit) and (y > target_position[1]-check_unit):
	...
	def generatePath(self, direction = None, fix_direction = False):
		...
		new_direction = None
		if self.type == self.TYPE_ARMOR or self.type == self.TYPE_POWER:
			new_directions = self.bfs()
			print "bfs"
		else: 
			new_directions = self.randomMove(new_direction)
...
class Player(Tank):
	...
	def getSuccessors(self,position):
		...
		new_pos_rect = cur_pos_rect.move(0, -16)
		if new_pos_rect.collidelist(self.level.obstacle_rects) == -1 and new_pos_rect.collidelist(other_tank_list) == -1:
			if x > 16:
				results.append(((x,y-16),self.DIR_UP))
		new_pos_rect = cur_pos_rect.move(16, 0)
		if new_pos_rect.collidelist(self.level.obstacle_rects) == -1 and new_pos_rect.collidelist(other_tank_list) == -1:
			if y < 384:	
				results.append(((x+16,y),self.DIR_RIGHT))
		new_pos_rect = cur_pos_rect.move(0, 16)
		if new_pos_rect.collidelist(self.level.obstacle_rects) == -1 and new_pos_rect.collidelist(other_tank_list) == -1:
			if x < 384:
				results.append(((x,y+16),self.DIR_DOWN))
		new_pos_rect = cur_pos_rect.move(-16, 0)
		if new_pos_rect.collidelist(self.level.obstacle_rects) == -1 and new_pos_rect.collidelist(other_tank_list) == -1:
			if y > 16:
				results.append(((x-16,y),self.DIR_LEFT))
		random.shuffle(results)
		return results
	...
	def heuristic(self,position):
		...
		for bonus in bonuses:
			if bonus == bonus.BONUS_GRENADE:
				return -800
			elif bonus == bonus.BONUS_SHOVEL:
				return -400
			elif bonus == bonus.BONUS_TIMER:
				return -200
			elif bonus == bonus.BONUS_HELMET:
				bonus_value = -50
			else:
				bonus_value = -10
		...
		heuristic_value = nearest_enemy - 5*lowest_enemy + bonus_value
	...
	def isGoalState(self,position):
		...
		for enemy in enemies:
			e_x = enemy.rect[0]
			e_y = enemy.rect[1]
			if abs(x - e_x) < 16 and abs(y - e_y) < 16:
				return True
		return False
	...
	def aStarSearch(self):
		...
		while not fringe.isEmpty():
			...
			if self.isGoalState(node):
				print "cost of actions: ",self.getCostOfActions(actions), "Heuristic: ", self.heuristic(node)
				return actions
