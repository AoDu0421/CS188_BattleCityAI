...
class Player(Tank):
	...
	def __init__(self, level, type, position = None, direction = None, filename = None):
		...
		# 1000 is duration between shots
		self.timer_uuid_fire = gtimer.add(1000, lambda :none)
	...
	def autoFire(self):
		...
		if self.direction == self.DIR_DOWN and self.rect.left > 176 and self.rect.left < 256:
			return
		if self.direction == self.DIR_RIGHT and self.rect.top > 352 and self.rect.left < 176:
			return
		if self.direction == self.DIR_LEFT and self.rect.top > 352 and self.rect.left > 256:
			return
		
		for enemy in enemies:
			if abs(enemy.rect.left - self.rect.left) < 13:
				if self.rect.top > enemy.rect.top:
					self.direction = self.DIR_UP
					self.fire()
				else:
					if not (self.rect.left > 176 and self.rect.left < 256):
						self.direction = self.DIR_DOWN
						self.fire()
			if abs(enemy.rect.top - self.rect.top) < 13:
				if self.rect.left > enemy.rect.left:
					if not (self.rect.top > 352 and self.rect.left < 256):
						self.direction = self.DIR_LEFT
						self.fire()
				else:
					if not (self.rect.top > 352 and self.rect.left < 176):
	...
	def aStarSearch(self):
		...
		while not fringe.isEmpty():
			...
			for curNode, curAction, cost in self.getSuccessors(node):
