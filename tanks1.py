...
class Player(Tank):
	...
	def atomfire(self, Tank):
		""" control to fire """
		if self.rect[0] == Tank.rect[0]:
			if self.rect[1] > Tank.rect[1]:
				originalDirection = self.direction
				self.direction = self.DIR_UP
				self.fire() 
				self.direction = originalDirection
			elif self.rect[1] < Tank.rect[1]:
				originalDirection = self.direction
