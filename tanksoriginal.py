	...
	def generatePath(self, direction = None, fix_direction = False):
		...
		for direction in directions:
			if direction == self.DIR_UP and y > 1:
				new_pos_rect = self.rect.move(0, -8)  # move the rect
				if new_pos_rect.collidelist(self.level.obstacle_rects) == -1:   # test if has a colliction
		...
		# try other three direction, then try the opposite direction at last
...
class Player(Tank):
	...
	def move(self, direction):
		...
		# move player  这里move player是通过坐标的改变实现
		if direction == self.DIR_UP:
