...
class Player(Tank):
	...
	def move(self):
		...
		target = pygame.Rect([3,5], [26, 26])

		if self.state != self.STATE_ALIVE or self.paused or self.paralised:
			return

		if self.path == []:
			#self.path = self.bfs(target)
			self.path = self.generatePath(None, True)
	...
	def bfs(self, target):
		"""generate path with bfs algrithom"""

		#generate 416 * 416 matrix
		searchedMap = [[] for irow in range(416)]
		for itimes in range(416):
			searchedMap[itimes] = [0 for jcolumn in range(416)]
		...
		flag = 0
		searchQueue = []
		searchRect = self.rect
		searchQueue.insert(0, searchRect)
		searchedMap[searchRect.left][searchRect.top] = 1
		succSearchRect = searchRect

		while len(searchQueue) != 0:
			searchRect = searchQueue.pop()
			x = int(round(searchRect.left / 16))
			y = int(round(searchRect.top / 16))
			flag = searchedMap[searchRect.left][searchRect.top]

			print "count: " + str(count)
			print "searchRect: " + str(searchRect)
			print "x: " + str(x)
			print "y: " + str(y)
			...
			if y > 1:
				print "if up"
				searchRectCopy = searchRect
				succSearchRect = searchRectCopy.move(0, -8)
				#if ((succSearchRect.left > target.left - 8) and (succSearchRect.left < target.left + 8)) and ((succSearchRect.top > target.top - 8) and (succSearchRect.top < target.top + 8)):
				if succSearchRect.left ==target.left and succSearchRect.top ==target.top:
					print"get it 1"
					break
				if searchedMap[succSearchRect.left][succSearchRect.top] == 0:
					if succSearchRect.collidelist(self.level.obstacle_rects) == -1:
						searchQueue.insert(0, succSearchRect)
						if count == 0:
							searchedMap[succSearchRect.left][succSearchRect.top] = 1
						else:
							searchedMap[succSearchRect.left][succSearchRect.top] = flag
							searchedMap[succSearchRect.left][succSearchRect.top + 1] = flag
							searchedMap[succSearchRect.left][succSearchRect.top + 2] = flag
							searchedMap[succSearchRect.left][succSearchRect.top + 3] = flag
							searchedMap[succSearchRect.left][succSearchRect.top + 4] = flag
							searchedMap[succSearchRect.left][succSearchRect.top + 5] = flag
							searchedMap[succSearchRect.left][succSearchRect.top + 6] = flag
							searchedMap[succSearchRect.left][succSearchRect.top + 7] = flag

			if x < 24:
				print "if right"
				searchRectCopy = searchRect
				succSearchRect = searchRectCopy.move(8, 0)
				#if ((succSearchRect.left > target.left - 8) and (succSearchRect.left < target.left + 8)) and ((succSearchRect.top > target.top - 8) and (succSearchRect.top < target.top + 8)):
				if succSearchRect.left ==target.left and succSearchRect.top ==target.top:
					print"get it 2"
					break
				if searchedMap[succSearchRect.left][succSearchRect.top] == 0:
					if succSearchRect.collidelist(self.level.obstacle_rects) == -1:
						searchQueue.insert(0, succSearchRect)
						if count == 0:
							searchedMap[succSearchRect.left][succSearchRect.top] = 2
						else:
							searchedMap[succSearchRect.left][succSearchRect.top] = flag
							searchedMap[succSearchRect.left - 1][succSearchRect.top] = flag
							searchedMap[succSearchRect.left - 2][succSearchRect.top] = flag
							searchedMap[succSearchRect.left - 3][succSearchRect.top] = flag
							searchedMap[succSearchRect.left - 4][succSearchRect.top] = flag
							searchedMap[succSearchRect.left - 5][succSearchRect.top] = flag
							searchedMap[succSearchRect.left - 6][succSearchRect.top] = flag
							searchedMap[succSearchRect.left - 7][succSearchRect.top] = flag
			if y < 24:
				print "if down"
				searchRectCopy = searchRect
				succSearchRect = searchRectCopy.move(0, 8)
				...
				if searchedMap[succSearchRect.left][succSearchRect.top] == 0:
					if succSearchRect.collidelist(self.level.obstacle_rects) == -1:
						searchQueue.insert(0, succSearchRect)
						if count == 0:
							searchedMap[succSearchRect.left][succSearchRect.top] = 3
						else:
							searchedMap[succSearchRect.left][succSearchRect.top] = flag
							searchedMap[succSearchRect.left][succSearchRect.top - 1] = flag
							searchedMap[succSearchRect.left][succSearchRect.top - 2] = flag
			...
			if x > 1:
				...
				if searchedMap[succSearchRect.left][succSearchRect.top] == 0:
					if succSearchRect.collidelist(self.level.obstacle_rects) == -1:
						searchQueue.insert(0, succSearchRect)
						if count == 0:
							searchedMap[succSearchRect.left][succSearchRect.top] = 4
						else:
							searchedMap[succSearchRect.left][succSearchRect.top] = flag
							searchedMap[succSearchRect.left + 1][succSearchRect.top] = flag
							searchedMap[succSearchRect.left + 2][succSearchRect.top] = flag
		...
		flag = searchedMap[searchRect.left][searchRect.top]
		if flag == 1:
			position.append([self.rect.left, self.rect.top - 8])
		elif flag == 2:
			position.append([self.rect.left + 8, self.rect.top])
		elif flag == 3:
			position.append([self.rect.left, self.rect.top + 8])
		elif flag == 4:
			position.append([self.rect.left - 8, self.rect.top])
