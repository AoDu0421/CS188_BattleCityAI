...
import os, pygame, time, random, uuid, sys, copy
...
class Player(Tank):
	...
	target = pygame.Rect([259, 102], [26, 26])
	...
	def moveForBfs(self, direction, searchRect):
		""" input a direction and a Rect, output a new Rect if this direction is available, return None if not available """

		# move player
		if direction == self.DIR_UP:
			new_position = [searchRect.left, searchRect.top - self.speed]
			if new_position[1] < 0:
				return None
		elif direction == self.DIR_RIGHT:
			new_position = [searchRect.left + self.speed, searchRect.top]
			if new_position[0] > (416 - 26):
				return None
		elif direction == self.DIR_DOWN:
			new_position = [searchRect.left, searchRect.top + self.speed]
	...
	def bfs(self, target):
		""" using bfs to find the next direction, then put this direction in move() """

		print "target: " + str(target)

		target1 = pygame.Rect([target.left - 1, target.top], [26, 26])
		target2 = pygame.Rect([target.left, target.top - 1], [26, 26])
		target3 = pygame.Rect([target.left + 1, target.top], [26, 26])
		target4 = pygame.Rect([target.left, target.top + 1], [26, 26])
		...
		# generate a queue to store different position
		searchQueue = []
		...
		while len(searchQueue) != 0:
			searchNode = searchQueue.pop()
			searchRect = searchNode[0]
			

			succSearchRect = self.moveForBfs(self.DIR_UP, searchRect)
			if succSearchRect != None:
				if searchedMap[succSearchRect.left][succSearchRect.top] == 0:
					searchPath = copy.deepcopy(searchNode[1])
					searchPath.insert(0, self.DIR_UP)
					searchQueue.insert(0, [succSearchRect, searchPath])
					searchedMap[succSearchRect.left][succSearchRect.top] = 1
					if succSearchRect == target:
						break
					if succSearchRect == target1:
						break
					if succSearchRect == target2:
						break
					if succSearchRect == target3:
						break
					if succSearchRect == target4:
						break

			succSearchRect = self.moveForBfs(self.DIR_RIGHT, searchRect)
			if succSearchRect != None:
				if searchedMap[succSearchRect.left][succSearchRect.top] == 0:
					searchPath = copy.deepcopy(searchNode[1])
					searchPath.insert(0, self.DIR_RIGHT)
					searchQueue.insert(0, [succSearchRect, searchPath])
					searchedMap[succSearchRect.left][succSearchRect.top] = 1
					if succSearchRect == target:
						break
			...
			succSearchRect = self.moveForBfs(self.DIR_DOWN, searchRect)
			if succSearchRect != None:
				if searchedMap[succSearchRect.left][succSearchRect.top] == 0:
					searchPath = copy.deepcopy(searchNode[1])
					searchPath.insert(0, self.DIR_DOWN)
					searchQueue.insert(0, [succSearchRect, searchPath])
					searchedMap[succSearchRect.left][succSearchRect.top] = 1
					if succSearchRect == target:
						break
			...
			succSearchRect = self.moveForBfs(self.DIR_LEFT, searchRect)
			if succSearchRect != None:
				if searchedMap[succSearchRect.left][succSearchRect.top] == 0:
					searchPath = copy.deepcopy(searchNode[1])
					searchPath.insert(0, self.DIR_LEFT)
					searchQueue.insert(0, [succSearchRect, searchPath])
					searchedMap[succSearchRect.left][succSearchRect.top] = 1
					if succSearchRect == target:
						break
		...
		if len(searchQueue) == 0:
			print "searchQueue is empty"
			return []
		else:
			print "we find target:"
			print searchQueue[0][0]
			return searchQueue[0][1]
...
class Game():
	...
	def reloadPlayers(self):
		...
		if len(players) == 0:
			...
			'''
			# using a tank to show target's position
			player = Player(
				self.level, 0, [259, 102], self.DIR_UP, (0, 0, 13*2, 13*2)
			)
			players.append(player)
			'''
		...
		while self.running:
			...
			for player in players:
				if player.state == player.STATE_ALIVE and not self.game_over and self.active:
					if len(enemies) != 0:
						if player.pressed[2] == True:
							player.testPath = player.bfs(enemies[0].rect)
							#player.testPath = player.bfs(player.target)
					...
					if len(player.testPath) != 0:
						player.move(player.testPath.pop())
